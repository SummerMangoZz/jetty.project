...
public class JSONTest
{
    ...
    {
        Map<String,String> map = new HashMap<>();
        map.put("str", "japanese: 健治");
        String jsonStr = JSON.toString(map);
        assertThat(jsonStr, is("{\"str\":\"japanese: 健治\"}"));
    }

    @Test
    public void testToString_Utf8_Encoded()
    {
        JSON.setStringEncoder(new JSONEscapedStringEncoder());
        Map<String,String> map = new HashMap<>();
        map.put("str", "japanese: 健治");
        String jsonStr = JSON.toString(map);
        assertThat(jsonStr, is("{\"str\":\"japanese: \\u5065\\u6cbb\"}"));
    }

    @Test
    public void testParse_Utf8_JsonEncoded()
    {
        String jsonStr = "{\"str\": \"japanese: \\u5065\\u6cbb\"}";
        Map map = (Map)JSON.parse(jsonStr);
        assertThat(map.get("str"), is("japanese: 健治"));
    }

    @Test
    public void testParse_Utf8_JavaEncoded()
    {
        String jsonStr = "{\"str\": \"japanese: \u5065\u6cbb\"}";
        Map map = (Map)JSON.parse(jsonStr);
        assertThat(map.get("str"), is("japanese: 健治"));
    }
